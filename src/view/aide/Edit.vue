    ...
    export default{
        ...
        methods:{
            ...
            idLoading(){
                ...
                that.$http.post(url,reqData).then(function (res) {
                    if(res.body.code == "200") {
                        ...
                        that.value7 = data.type
                        that.Thetitle = data.title
                        that.content  = data.content
                        contents = data.content
                        that.imageUrls  = data.assistImageurls
                        if (that.imageUrls != '') {
                            var str = that.imageUrls
                            var arr=str.split(",");
                            //输出结果 
                            that.imageUrls  = arr;
                            for (var i = 0; i < that.imageUrls.length; i++) {
                                that.imageUrls[i] = `http://hellobaobei.oss-cn-shanghai.aliyuncs.com/static/images/classroom/${this.imageUrls[i]}` 
                            }
                        }
                        this.sunEditor();
                    }
            ...
            NewAdd:function(){
                ...
                var url          = 'classroomAssist/update'
                var reqData      = {}
                reqData.assistId = this.assist.assistId
                ...
                this.$http.post(url, reqData).then(function (res){
                    if(res.body.code == '200'){
                        this.$notify({title: '成功',message: '课堂修改成功',type: 'success'});
                        this.doCancel();
                        this.successaide();
                    }
                })
