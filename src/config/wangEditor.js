...
var config = {
    ...
    // 默认菜单配置
    menus: ['head', 'bold', 'italic', 'underline', 'strikeThrough', 'foreColor', 'backColor', 'link', 'list', 'justify', 'quote', 'emoticon', 'image', 'table', 'video'],
...
function UploadImg(editor) {
    ...
    this.columns=3;
    this.dir='static/images/schoolHead/';
    this.bjdir='static/images/schoolBackGroundImg/';
    this.items=[];
    this.region='cn-hangzhou';
    this.bucket='hellobaobei';
    ...
    this.fileType= ["jpg","png","bmp","jpeg",'gif'];
...
UploadImg.prototype = {
    ...
    state_Change:function state_Change(){
        var self = this
        setTimeout(function () {
            console.log(JSON.stringify(self.xmlhttp.response));
            self.result =  eval ("(" + self.xmlhttp.response + ")");
            /*self.result = self.xmlhttp.response*/

            console.log(self.xmlhttp.response);
        }, 300);
        console.log(JSON.stringify(this.xmlhttp.response));
    ...
    onFiledate:function onFiledate(files) {
        ...
        if(fileList.length > this.columns || totalNum > this.columns ){
            ...
            const _this = this;
            //babyAdminServer/
            _this.loadXMLDoc('http://www.hellobaobei.com.cn:8888/babyAdminServer/ali/sts')
            /*_this.$http.get('ali/sts').then((response) => {*/

                /*const result = response.body.data*/
                setTimeout(function () {
                    const result = _this.result.data
                ...
                for(var i = 0;i<fileList.length;i++){
                    ...
                    client.multipartUpload(storeAs, file).then(function (result) {
                        ...
                        console.log(_this.bgimgUrl);
                        _this.insertLinkImg(_this.bgimgUrl);
                    }).catch(function (err) {
                        //console.log(err);
                    });
                }
    ...
    validateUp:function validateUp(files){
    ...
    inArray:function inArray(search,arr) {
